<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>MyResep Profile</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-white text-gray-900 font-sans">

    <!-- Navbar -->
    <nav class="flex justify-between items-center px-8 py-4 border-b bg-white shadow-sm">
        <div class="flex items-center space-x-2">
            <img src="/images/logo.png" alt="MyResep Logo" class="h-8">
            <span class="font-bold text-lg">MyResep</span>
        </div>
        <div class="flex items-center space-x-6 text-sm">
            <a href="/dashboard" class="hover:underline">Beranda</a>
            <a href="#" class="hover:underline">Favorit</a>
            <a href="/profile" class="hover:underline">{{ $user->name }}</a>
        </div>
    </nav>

    <!-- Profile Section -->
    <section class="bg-green-900 text-white py-10 px-4 text-center">
        <h1 class="text-2xl font-bold">Profile Information</h1>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="bg-green-800 hover:bg-green-700 mt-4 px-4 py-2 rounded-md">Logout</button>
        </form>
    </section>

    <!-- Flash Message -->
    @if(session('success'))
        <div class="max-w-2xl mx-auto mt-6">
            <div class="bg-green-100 border border-green-300 text-green-800 px-4 py-3 rounded">
                {{ session('success') }}
            </div>
        </div>
    @endif

    <!-- Profile Content -->
    <section class="flex justify-center items-start px-6 py-12">
        <div class="flex flex-col md:flex-row items-center space-x-8">
            <!-- Profile Image -->
            <img 
                src="{{ $user->profile_image ? asset('storage/' . $user->profile_image) : asset('images/user-icon.png') }}" 
                alt="User Icon" 
                class="h-48 w-48 rounded-full object-cover mb-6 md:mb-0"
            >

            <!-- Profile Details -->
            <div class="space-y-4">
                <div class="flex items-center space-x-4">
                    <span class="text-lime-500 text-xl">👤</span>
                    <div>
                        <p class="text-sm text-gray-600">Username</p>
                        <p class="font-semibold">{{ $user->name }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-lime-500 text-xl">🆔</span>
                    <div>
                        <p class="text-sm text-gray-600">MyResep ID</p>
                        <p class="font-semibold">MR-{{ str_pad($user->id, 3, '0', STR_PAD_LEFT) }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-lime-500 text-xl">📧</span>
                    <div>
                        <p class="text-sm text-gray-600">Email</p>
                        <p class="font-semibold">{{ $user->email }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-lime-500 text-xl">🍃</span>
                    <div>
                        <p class="text-sm text-gray-600">Bio</p>
                        <p class="font-semibold">{{ $user->bio ?? 'Belum diisi' }}</p>
                    </div>
                </div>

                <a href="{{ route('profile.edit') }}" class="bg-green-900 text-white px-6 py-2 rounded-md mt-4 inline-block text-center">Edit</a>
            </div>
        </div>
    </section>

    <!-- Footer -->
    <footer class="border-t py-6 mt-10 text-sm text-center text-gray-500">
        <p class="mb-2">© 2025 MyResep</p>
        <div class="space-x-4">
            <a href="#" class="hover:underline">About us</a>
            <a href="#" class="hover:underline">Contact Us</a>
        </div>
    </footer>

</body>
</html>
