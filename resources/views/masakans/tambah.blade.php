...
<section class="bg-gray-50 rounded p-4 flex flex-col gap-6">
    ...
    <form action="{{ route('masakans.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 :grid-cols-2 gap-10 mt-10">
            <div>
                <label for="de7tail_Resep" class="block text-xs font-semibold mb-1">Langkah-langkah</label>
                <textarea id="detail_Resep" name="detail_Resep" rows="8" placeholder="Tuliskan langkah-langkah membuat resep" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-green-700">{{ old('detail_Resep') }}</textarea>
                @error('detail_Resep')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
