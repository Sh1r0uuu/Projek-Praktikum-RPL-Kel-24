...
<body class="bg-white text-gray-900 font-sans">
    ...
    <section class="bg-green-900 text-white text-center py-12 px-4">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between">
            <h1 class="text-3xl font-bold mb-2 md:mb-0 text-left">Search Result</h1>
            <form class="flex" action="{{ route('masakans.search') }}" method="GET">
                <input type="text" name="query" placeholder="Cari Resep" class="px-4 py-2 rounded-md text-black w-64" value="{{ $searchQuery ?? '' }}">
    ...
    <section class="px-6 py-8">
        @if(isset($searchQuery) && $searchQuery)
            <h2 class="text-xl font-bold mb-6">Hasil Pencarian untuk "{{ $searchQuery }}"</h2>
        @else
            <h2 class="text-xl font-bold mb-6">Semua Resep</h2>
        @endif
...
@endif
            ...
            @empty
                <p class="text-center text-sm text-gray-500 col-span-3">
                    @if(isset($searchQuery) && $searchQuery)
