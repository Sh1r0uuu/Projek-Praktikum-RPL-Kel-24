...
<head>
    <meta charset="UTF-8">
    <title>Edit Resep - MyResep</title>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
    ...
    <div class="flex items-center space-x-2 mb-4">
            <h1 class="text-2xl font-bold mb">Edit Resep</h1>
    </div>
        <p class="text-sm text-gray-700">Ubah informasi resep yang sudah ada</p>
<div class="flex items-center space-x-2 mt-40">
        <h2 class="text-lg font-bold mb-2">Edit Data Resep</h2>
</div>
    </section>
    ...
    <section class="bg-gray-50 rounded p-4 flex flex-col gap-6">
        ...
        <div class="bg-gray-200 rounded h-44 flex items-center justify-center">
            <div id="image-preview">
                @if($masakan->gambar_Masakan)
                    <img src="{{ $masakan->gambar_Masakan }}" alt="Preview" class="max-h-44 max-w-full object-contain">
                @else
        ...
        <!-- Form Start -->
        <form action="{{ route('masakans.update', $masakan->id_Masakan) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="gambar_Masakan" name="gambar_Masakan" value="{{ old('gambar_Masakan', $masakan->gambar_Masakan) }}" placeholder="URL gambar"
            ...
            <div>
                ...
                <input type="text" id="nama_Masakan" name="nama_Masakan" value="{{ old('nama_Masakan', $masakan->nama_Masakan) }}" placeholder="Judul resep" required
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-green-700" />
                @error('nama_Masakan')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="relative">
                ...
                <select id="kategori_Masakan" name="kategori_Masakan" required
                    ...
                    <option value="pembuka" {{ (old('kategori_Masakan', $masakan->kategori_Masakan) == 'pembuka') ? 'selected' : '' }}>Pembuka</option>
                    <option value="utama" {{ (old('kategori_Masakan', $masakan->kategori_Masakan) == 'utama') ? 'selected' : '' }}>Utama</option>
                    <option value="penutup" {{ (old('kategori_Masakan', $masakan->kategori_Masakan) == 'penutup') ? 'selected' : '' }}>Penutup</option>
                </select>
            ...
            <div>
                ...
                <textarea id="deskripsi_Resep" name="deskripsi_Resep" rows="3" placeholder="Deskripsi resep" required
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-green-700">{{ old('deskripsi_Resep', $masakan->deskripsi_Resep) }}</textarea>
            ...
            <div>
                ...
                <textarea id="bahan_Memasak" name="bahan_Memasak" rows="6" placeholder="Tuliskan alat dan bahan yang diperlukan" required
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-green-700">{{ old('bahan_Memasak', $masakan->bahan_Memasak) }}</textarea>
                @error('bahan_Memasak')
                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>

            <div class="grid grid-cols-1 :grid-cols-2 gap-10 mt-10">
                <div>
                    <label for="detail_Resep" class="block text-xs font-semibold mb-1">Langkah-langkah</label>
                    <textarea id="detail_Resep" name="detail_Resep" rows="8" placeholder="Tuliskan langkah-langkah membuat resep" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-green-700">{{ old('detail_Resep', $masakan->detail_Resep) }}</textarea>
                    @error('detail_Resep')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="flex space-x-4 mt-6">
                        ...
                        <button type="submit" class="flex-1 bg-green-900 hover:bg-green-800 text-white rounded px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-green-700">Simpan Perubahan</button>
                    </div>
                </div>
            </div>
            </form>
