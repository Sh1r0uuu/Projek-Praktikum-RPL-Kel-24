    ...
    <section class="px-8 py-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <ul class="space-y-2">
                    <li><strong>Waktu Masak:</strong> {{ $recipe->time ?? $recipe->cooking_time ?? 'Tidak tersedia' }} menit</li>
                    <li><strong>Tingkat Kesulitan:</strong> {{ $recipe->difficulty ?? 'Tidak diketahui' }}</li>
                    <li><strong>Jenis Makanan:</strong> {{ $recipe->type ?? $recipe->category ?? 'Tidak diketahui' }}</li>
                </ul>
                ...
                @if ($recipe->image)
                    <div class="mt-6">
                        ...
                        <img src="{{ asset('storage/' . $recipe->image) }}" alt="{{ $recipe->title }}" class="w-full h-auto rounded-md">
                    </div>
                @else
